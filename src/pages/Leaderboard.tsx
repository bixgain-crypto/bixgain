    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="glass rounded-2xl overflow-hidden">
          {/* Desktop header */}
          <div className="hidden sm:grid grid-cols-[60px_1fr_100px_100px] gap-3 px-4 py-3 border-b border-border/60 text-xs uppercase tracking-wider text-muted-foreground">
            <span>Rank</span>
            <span>User</span>
            <span>XP</span>
            <span>Level</span>
          ...
          {loading.leaderboard && !activeData ? (
            ...
            <div className="divide-y divide-border/50">
              {rows.map((row) => (
                <div
                  key={row.user_id}
                  className={`px-3 py-3 sm:grid sm:grid-cols-[60px_1fr_100px_100px] sm:gap-3 sm:items-center ${
                ...
                >
                  ...
                  <div className="flex items-center justify-between sm:contents">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className="font-mono text-sm text-muted-foreground shrink-0">{`#${row.rank}`}</span>
                      <div className="min-w-0">
                        <p className="truncate font-medium text-sm">{row.username}</p>
                        {row.is_current_user && <p className="text-[10px] text-primary leading-tight">You</p>}
                      </div>
                    </div>
