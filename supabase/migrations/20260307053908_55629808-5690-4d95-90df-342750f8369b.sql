...
BEGIN
  ...
  IF _last_claim_at IS NOT NULL AND _last_claim_at > (now() - INTERVAL '3 hours') THEN
    _next_claim_at := _last_claim_at + INTERVAL '3 hours';
    RAISE EXCEPTION 'Boost already claimed. Try again later.'
      USING ERRCODE = 'P0001',
  ...
  _next_claim_at := now() + INTERVAL '3 hours';
