-- Harden daily reward + activities defaults to prevent null activity_type inserts

-- Ensure activities.activity_type always gets a safe default.
ALTER TABLE public.activities
  ALTER COLUMN activity_type SET DEFAULT 'custom';

CREATE OR REPLACE FUNCTION public.activities_apply_defaults()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  NEW.activity_type := COALESCE(NEW.activity_type, 'custom'::public.activity_type);
...
DROP TRIGGER IF EXISTS trg_activities_apply_defaults ON public.activities;
CREATE TRIGGER trg_activities_apply_defaults
BEFORE INSERT ON public.activities
FOR EACH ROW
EXECUTE FUNCTION public.activities_apply_defaults();

-- Replace every overload of public.claim_daily_reward with one canonical pair.
DO $$
DECLARE
  _fn REGPROCEDURE;
BEGIN
  FOR _fn IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'claim_daily_reward'
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS %s;', _fn);
  END LOOP;
END;
$$;
